...
/* harmony export */   "megaMenuChildSetTitle": () => (/* binding */ megaMenuChildSetTitle)
/* harmony export */ });
...
function setActiveColors() {
  ...
  for (var i = 0; i < openMegaMenuParents.length; i++) {
    var parentLi = openMegaMenuParents[i].parentElement; // The bottom border got moved from the <li> to the span inside so we check to see if a span is present to reset

    var tarSpan = parentLi.getElementsByTagName('span')[0];

    if (typeof tarSpan !== "undefined") {
      tarSpan.classList.remove('madrone-active-trail');
    }

    parentLi.classList.remove('tw-text-osuorange', 'hover-tw-text-osuorange');
...
function megaMenuToggle() {
  ...
  var megaMenuParent = document.getElementsByClassName('mega-menu-parent no-link'); // Adds event listener to close MegaMenu
...
function megaMenuChildSetTitle() {
  var megaMenuChildArr = document.querySelectorAll('.mega-menu-child > a');

  for (var i = 0; i < megaMenuChildArr.length; i++) {
    var v1 = megaMenuChildArr[i];
